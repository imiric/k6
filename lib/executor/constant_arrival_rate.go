...
func (car ConstantArrivalRate) Run(ctx context.Context, out chan<- stats.SampleContainer) (err error) { //nolint:funlen
	...
	progresFn := func() (float64, []string) {
		...
		return math.Min(1, float64(spent)/float64(duration)), []string{
			fmt.Sprintf(vusFmt+"/"+vusFmt+" VUs", currentInitialisedVUs-vusInBuffer, currentInitialisedVUs),
			fmt.Sprintf(pb.GetFixedLengthFloatFormat(arrivalRatePerSec, 0)+" iters/s", arrivalRatePerSec),
		}
	}
