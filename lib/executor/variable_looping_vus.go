...
func (vlv VariableLoopingVUs) Run(ctx context.Context, out chan<- stats.SampleContainer) (err error) {
	...
	progresFn := func() (float64, []string) {
		...
		return float64(spent) / float64(regularDuration), []string{
			fmt.Sprintf(vusFmt+"/"+vusFmt+" VUs", currentlyActiveVUs, maxVUs),
			fmt.Sprintf("%s/%s", pb.GetFixedLengthDuration(spent, regularDuration), regularDuration),
		}
	}
